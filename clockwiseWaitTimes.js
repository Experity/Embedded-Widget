function beginWaitTimeQuerying(waitFetchObjects, timeBetweenUpdates) {
  ...
  function determineTimeToAdd(waitFetchObject, data) {
    ...
    var rawValue = rawWaitValue(waitFetchObject.timeType, waitFetchObject.queueId, data)
  ...
  function rawWaitValue(timeType, queueId, data) {
    ...
    if(timeType === 'queueWait') return determineQueueTimeToAdd(queueId, data).current_wait;
    if(timeType === 'queuePatientsInLine') return determineQueueTimeToAdd(queueId, data).current_patients_in_line;
    if(timeType === 'queueWaitRange') return determineQueueTimeToAdd(queueId, data).current_wait_range;
    return 'Invalid timeType';
  }

  // This is a helper function that finds the desired queue to grab wait
  // information from
  function determineQueueTimeToAdd(queueId, data) {
    var numberOfQueues = data.appointment_queues.length;
    for(var k = 0; k < numberOfQueues; k++) {
      if(data.appointment_queues[k].queue_id === queueId) {
